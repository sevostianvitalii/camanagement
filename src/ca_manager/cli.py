...
def validate(check: str, path: Path, baseline_path: Path):
    ...
    console.print(
        Panel(f"[bold blue]Validating policies in {path}[/bold blue]", title="CA Policy Validation")
    )
...
@click.option(
    "--dry-run/--no-dry-run", default=True, help="If true, validate but do not apply changes"
)
