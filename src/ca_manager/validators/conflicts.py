...
def are_policies_identical(p1: dict, p2: dict) -> bool:
    ...
    if (
        set(users1.get("includeGroups", [])) != set(users2.get("includeGroups", []))
        or set(users1.get("excludeGroups", [])) != set(users2.get("excludeGroups", []))
        or set(users1.get("includeUsers", [])) != set(users2.get("includeUsers", []))
    ):
        return False
...
def _is_system_group(group_name: str) -> bool:
    ...
    exclude_patterns = ["Service-", "System-", "Sync-", "BreakGlass", "Emergency", "AAD-Device"]
